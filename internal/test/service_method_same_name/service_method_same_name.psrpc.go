...
func (c *echoClient) Echo(ctx context.Context, req *Msg, opts ...psrpc.RequestOption) (*Msg, error) {
	return psrpc.RequestSingle[*Msg](ctx, c.client, "Echo", "", req, opts...)
}
...
// NewEchoServer builds a RPCServer that will route requests
...
func NewEchoServer(serverID string, svc EchoServerImpl, bus psrpc.MessageBus, opts ...psrpc.ServerOption) (EchoServer, error) {
	s := psrpc.NewRPCServer("Echo", serverID, bus, opts...)

	var err error
	err = psrpc.RegisterHandler(s, "Echo", "", svc.Echo, nil)
	if err != nil {
		s.Close(false)
		return nil, err
	}
